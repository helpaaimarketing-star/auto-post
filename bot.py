...
class DiscordBot:
    ...
    def _register_commands(self):
        ...
        async def analyze_command(interaction: discord.Interaction, link: str):
            await interaction.response.defer()
            ana_ch = await get_or_create_channel(interaction.guild, Config.ANALYSIS_CHANNEL_NAME)
            ...
            try:
                ...
                for idx, emb in enumerate(post_embeds):
                    ...
                    await ana_ch.send(embed=emb, view=view)
                    await asyncio.sleep(0.3)
        ...
        @app_commands.describe(
            ...
            template_code="Approved template ka 4-digit code (optional)",
            count="Kitni posts generate karni hain (optional, default 1)",
        )
        ...
        @app_commands.choices(platform=[
            ...
            app_commands.Choice(name="Twitter/X — Tweets, Threads", value="twitter"),
            app_commands.Choice(name="LinkedIn  — B2B, Articles", value="linkedin"),
            app_commands.Choice(name="TikTok    — Short Video & script", value="tiktok"),
            app_commands.Choice(name="Threads   — Casual discussion", value="threads"),
            app_commands.Choice(name="YouTube Shorts — Video & Script", value="youtube_shorts"),
        ])
        async def build_command(interaction: discord.Interaction, order_id: str,
            ...
            if template_code:
                ...
                cached_tpl = cache.get(template_code)
                if not cached_tpl:
                    await interaction.followup.send(f"❌ Template code `{template_code}` cache mein nahi mila ya expire ho chuka hai.")
                    return
            ...
            if cached_tpl:
                ...
                posts = await asyncio.to_thread(
                    self.ai.generate_posts_from_template, business, niche, platform, cached_tpl["template"], count or 1
                )
                
                # Format bulk packages
                for idx, post in enumerate(posts):
                    # Generate free Pollinations image for each post
                    style = post.get("style", "minimalist/clean")
                    file_type = post.get("file_type", "PNG")
                    ...
                    if post.get("generated_image_url"):
                        embed.set_image(url=post["generated_image_url"])
                ...
                # Fallback standard post from scratch
                ...
                # Check for visual generation
                style = post_data.get("style", "Post")
                img_prompt = post_data.get("image_prompt", "")
                if img_prompt:
                    encoded = urllib.parse.quote(f"hyper-realistic, high-end commercial photography, professional marketing asset. {img_prompt}")
                    seed = random.randint(1, 1000000)
                    image_url = f"https://image.pollinations.ai/prompt/{encoded}?width=1080&height=1080&nologo=true&private=true&seed={seed}"
                    post_data["generated_image_url"] = image_url
