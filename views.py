...
async def send_post_embeds_with_images(channel, analysis_emb, post_embeds, post_data, template_codes, business_name, niche):
    """
    Send post embeds then async-download images and follow up.
    Keeps bot responsive — no blocking on Pollinations.
    """
    import aiohttp
    if analysis_emb:
        await channel.send(embed=analysis_emb)
    ...
    for idx, emb in enumerate(post_embeds):
        ...
        # Send embed immediately (no image yet)
        msg = await channel.send(embed=emb, view=view)
        await asyncio.sleep(0.3)

        # Now async-download the image and edit the message
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(image_url, timeout=aiohttp.ClientTimeout(total=90)) as resp:
                        if resp.status == 200:
                            ...
                            ext = "jpg" if file_type.upper() == "JPEG" else "png"
                            fname = f"template_{idx+1}_{code}.{ext}"
                            # Edit embed to use attachment
                            new_emb = emb.copy()
                            new_emb.set_image(url=f"attachment://{fname}")
                            await msg.edit(
                                embed=new_emb,
                                attachments=[discord.File(fp=io.BytesIO(data), filename=fname)],
                            )
            except Exception as e:
                logging.getLogger("views").warning(f"Async image download failed for template {idx+1}: {e}")
...
def build_post_template_embeds(post_data: dict) -> tuple[discord.Embed | None, list[discord.Embed], list[str]]:
    ...
    for i, tpl in enumerate(post_data.get("templates", [])):
        ...
        # Pollinations image generation — download & attach as file so it always renders
        ...
        if img_prompt:
            ...
            # Just set URL now — async download happens after Discord ack
